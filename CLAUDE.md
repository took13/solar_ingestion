...
## 3. Huawei API Rate Limit (Section 4.2 of PDF)

**ห้ามจำพลาด — ทำคำนวณก่อนเสนอ schedule change ทุกครั้ง:**

```
getDevRealKpi cap     = ∑ Roundup(N_devtype / 100)  per account, per 5 min
getStationRealKpi cap = Roundup(N_plants / 100)      per account, per 5 min
getDevHistoryKpi cap  = ∑ (N_devtype / 60 / 10)      per account, per second (rate, not count)
getAlarmList cap      = MAX( Roundup(N_plants/100), ∑ Roundup(N_devtype/100) ) per 30 min
```

### Current per-account usage (rounded to 5-min slot)

| Account | API | Devices | Calls/5min | Cap | Headroom |
|---|---|---|---|---|---|
| 1 | getDevRealKpi dt=10 | ~15 EMI | 1 | 1 | 0 (edge) |
| 2 | getDevRealKpi dt=17 | ~15 Meter | 1 | 1 | 0 (edge) |
| 3 | getStationRealKpi | 15 plants | 1 | 1 | 0 (edge) |
| 4 | getDevRealKpi dt=1 | 238 inverters | 3 | 3 | 0 (edge) |

**Notes:**
- ทุก account อยู่ที่ edge ของ cap → ห้าม "เพิ่ม batch" หรือ "เร่ง trigger" โดยไม่ recalculate
- ถ้าจะเพิ่ม alarm/event ingestion (Phase 2) → ใช้ account 3 ช่วงที่ idle (NIGHT) เพราะ realtime ของ acc 3 อยู่ Day mode เท่านั้น
...
**Step 2: Capacity calculation (ต้องคำนวณก่อน)**
```
chunks        = total_window_minutes / chunk_minutes (default 60)
total_calls   = chunks × CEILING(inverter_count / batch_size)
duration_est  = total_calls × ~3 sec/call (AccountRateGate pacing)
```
ตัวอย่าง: NE=49936764 (27 inverters, batch_size=10) × 180 วัน × 24 hr/day = 4320 chunks × 3 batches = **12,960 calls × 3 sec ≈ 11 ชั่วโมง** total
→ spread across 3 nights (00:00–05:00 each) → feasible
...
- ใช้ `AccountRateGate` pacing ของระบบ (3+ sec ระหว่าง call)
